...
var (
	...
	reComp           = regexp.MustCompile(`(?i)\*\*(?:Comp|Comp Estimate)\*\*\s*\|\s*(.+)`)
...
func NormalizeStatus(raw string) string {
	...
	case strings.Contains(s, "discarded") || s == "closed" || s == "cancelled" || s == "canceled" ||
		...
		return "discarded"
	case strings.Contains(s, "evaluated") || s == "conditional" || s == "hold" || s == "on hold" || s == "monitor" || s == "review" || s == "evaluate" || s == "verify":
		return "evaluated"
	default:
		return s
	}
