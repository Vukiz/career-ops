...
function validateStatus(status) {
  ...
  const aliases = {
    ...
    'rejected': 'Rejected',
    'skip': 'SKIP',
    'conditional': 'Evaluated', 'hold': 'Evaluated', 'on hold': 'Evaluated',
    'monitor': 'Evaluated', 'evaluate': 'Evaluated', 'review': 'Evaluated', 'verify': 'Evaluated',
    'geo blocker': 'SKIP',
  };
...
function parseTsvContent(content, filename) {
  ...
  if (content.startsWith('|')) {
    ...
    const col4LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|skip|closed|cancelled|canceled|duplicate|repost|conditional|hold|on hold|monitor|review|evaluate|verify)/i.test(col4);
    const col5LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|skip|closed|cancelled|canceled|duplicate|repost|conditional|hold|on hold|monitor|review|evaluate|verify)/i.test(col5);
